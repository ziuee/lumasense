package me.luma.client.management.gui.material.components.components.sub;
...
import me.luma.client.management.gui.material.components.components.Button;
...
public class Slider extends Component {
	...
	public void renderComponent() {
		Gui.drawRect(parent.parent.getX() + 2, parent.parent.getY() + offset, parent.parent.getX() + parent.parent.getWidth(), parent.parent.getY() + offset + 12, this.hovered ? 0xFF222222 : 0xFF111111);
		 final int drag = (int)(this.set.getSliderValue() / this.set.getSliderValue().MAX_VALUE * this.parent.parent.getWidth());
		...
		Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(this.set.getName() + ": " + this.set.getSliderValue() , (parent.parent.getX()* 2 + 15), (parent.parent.getY() + offset + 2) * 2 + 5, -1);
	...
	public void updateComponent(int mouseX, int mouseY) {
		...
		double min = set.getSliderValue().MIN_VALUE;
		double max = set.getSliderValue().MAX_VALUE;
		
		renderWidth = (88) * (set.getSliderValue() - min) / (max - min);
		
		if (dragging) {
			if (diff == 0) {
				set.setValue(set.getSliderValue().MIN_VALUE);
			}
			else {
