...
import me.luma.client.management.utils.scaffold.ScaffoldUtils;
...
public class ScaffoldTest extends Module {
	...
	int lastItem = -1;
    boolean down = false;
    TimerTEst towerTimer = new TimerTEst();
    TimerTEst downwardsTimer = new TimerTEst();
    TimerTEst slowDownTimer = new TimerTEst();
    ...
    SettingBoolean swing;
    
	public ScaffoldTest() {
		super("ScaffoldTest", 0, Category.MOVEMENT);
		
		tower = new SettingBoolean("Tower", this, true);
		swing = new SettingBoolean("Swing", this, true);
	}
	@Override
    ...
    public void onUpdate(EventUpdate event) {
        ...
        if (event.isPre()) {
            ...
            //mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, mc.thePlayer.getHeldItem(), lastBlockCache.position, lastBlockCache.facing, new Vec(0.5, 0.5, 0.5));
            ...
            int time = 30;
            if (ClientLoader.loaderInstance.moduleManager.getModule("Speed").isToggled())
                time = 10;
    ...
    public void onRender(Event2D e) {
        ...
        mc.fontRendererObj.drawStringWithShadow(blockCount + "", (sr.getScaledWidth() / 2 - -10) - mc.fontRendererObj.getStringWidth(blockCount + "") / 2, (sr.getScaledHeight() / 2 + 30) + -21, color.getRGB());
    }
