...
public class Gui
{
    ...
    public static void drawRect(float left, float top, float right, float bottom, final Color color) {
        if (left < right) {
            final float tessellator = left;
            left = right;
            right = tessellator;
        }
        if (top < bottom) {
            final float tessellator = top;
            top = bottom;
            bottom = tessellator;
        }
        final Tessellator tessellator2 = Tessellator.getInstance();
        final WorldRenderer worldrenderer = tessellator2.getWorldRenderer();
        GlStateManager.enableBlend();
        GlStateManager.disableTexture2D();
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
    ...
    public static void drawRect(double x, double y, double d, double e, int color) // Luar ClickGui
    {
        if (x < d)
        {
            double i = x;
