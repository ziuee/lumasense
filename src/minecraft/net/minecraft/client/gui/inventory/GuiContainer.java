...
public abstract class GuiContainer extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        this.buttonList.add(new GuiButton(2272, 0, 0, 140, 20, "Disable Aura"));
        this.buttonList.add(new GuiButton(2273, 0, 21, 140, 20, "Disable Chest Stealer"));
    ...
    /*public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
    	scaling = animUtil.easeOut(0, 0, 1, .5);
    	scaling = MathHelper.clamp_double(scaling, 0, 1);
        this.drawDefaultBackground();
        int i = this.guiLeft;
        int j = this.guiTop;
        ...
        if (mouseButton == 0) {
            ...
            while(var4.hasNext()) {
               ...
               if (button.mousePressed(this.mc, mouseX, mouseY)) {
                  ...
                  Module mod;
                  if (button.id == 2272) {
                     mod = ClientLoader.loaderInstance.moduleManager.getModule("KillAura");
                     if (ClientLoader.loaderInstance.moduleManager.getModule("KillAura").isToggled()) {
                        mod.toggle();
                     }
                  } else if (button.id == 2273) {
                	  mod = ClientLoader.loaderInstance.moduleManager.getModule("ChestStealer");
                      if (ClientLoader.loaderInstance.moduleManager.getModule("ChestStealer").isToggled()) {
                        mod.toggle();
                     }
                  } else if (button.id == 2274) {
                	  mod = ClientLoader.loaderInstance.moduleManager.getModule("InvManager");
                      if (ClientLoader.loaderInstance.moduleManager.getModule("InvManager").isToggled()) {
                        mod.toggle();
                     }
                  }
               }
