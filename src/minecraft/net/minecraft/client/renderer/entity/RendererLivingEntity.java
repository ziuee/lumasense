...
import me.luma.client.management.module.impl.render.Chams;
import me.luma.client.management.module.impl.render.CustomHitColor;
import me.luma.client.management.module.impl.render.ESP;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        //EventRenderEntity eventRenderEntity = new EventRenderEntity(State.PRE, entity, x, y, z, partialTicks);
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
            ...
            try {
                ...
                if (this.renderOutlines) {
                    ...
                    if (active) {
//                        if (Minecraft.getMinecraft().getNetHandler().getPlayerInfo(entity.getUniqueID()) == null)
//                            active = false;
                    ...
                    if (active) {
                        this.renderModel(entity, f6, f5, f7, f2, f8, 0.0625F, false);
                        ...
                        RenderUtils.renderFour(new Color(ESP.colorRed.getSliderValue().floatValue() / 255F,
                        ESP.colorGreen.getSliderValue().floatValue() / 255F,
                        ESP.colorBlue.getSliderValue().floatValue() / 255F).getRGB());
                        this.renderModel(entity, f6, f5, f7, f2, f8, 0.0625F, true);
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_, boolean isOutline) {
		boolean flag = !entitylivingbaseIn.isInvisible();
		boolean flag1 = !flag && !entitylivingbaseIn.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer);
		...
		boolean undo = ClientLoader.loaderInstance.moduleManager.getModule("Chams").isToggled() && !isOutline;
		...
		GlStateManager.blendFunc(770, 771);
		GlStateManager.alphaFunc(516, 0.003921569F);
		} else if (undo &= entitylivingbaseIn instanceof EntityOtherPlayerMP) {
		GL11.glPushMatrix();
		GL11.glDepthMask(false);
		if (Chams.chamsMode.getArraListValue().equalsIgnoreCase("Flat")) {
		    GL11.glEnable(GL11.GL_BLEND);
		} else if (Chams.chamsMode.getArraListValue().equalsIgnoreCase("Blend")) {
		...
		Color c = Color.getHSBColor(Chams.hueWalls.getSliderValue().floatValue(), 1.0F, 1.0F);
		float red = c.getRed() / 255.0F;
		float blue = c.getBlue() / 255.0F;
		float green = c.getGreen() / 255.0F;
		if (Chams.rainbowChams.getBooleanValue()) {
			int col = Chams.getRainbow(-6500, -15);
			float red2 = (col >> 16 & 255) / 255.0F;
			float green2 = (col >> 8 & 255) / 255.0F;
			float blue2 = (col & 255) / 255.0F;
		 	GL11.glColor4f(red2, blue2, green2, Chams.alphaWalls.getSliderValue().floatValue());
		} else
			GL11.glColor4f(red, blue, green, Chams.alphaWalls.getSliderValue().floatValue());
		}
		...
		if (flag1) {
			...
			GL11.glPolygonOffset(1.0F, 1000000.0F);
			Color c = Color.getHSBColor(Chams.hue.getSliderValue().floatValue(), 1.0F, 1.0F);
			float red = c.getRed() / 255.0F;
			float blue = c.getBlue() / 255.0F;
			float green = c.getGreen() / 255.0F;
			if (Chams.rainbowChams.getBooleanValue()) {
			...
			float green2 = (col >> 8 & 255) / 255.0F;
			float blue2 = (col & 255) / 255.0F;
			GL11.glColor4f(red2, blue2, green2, Chams.alpha.getSliderValue().floatValue());
		} else
			GL11.glColor4f(red, blue, green, Chams.alpha.getSliderValue().floatValue());
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures)
    {
        ...
        else
        {
            ...
            if (flag1) {
                if (ClientLoader.loaderInstance.moduleManager.getModule("CustomHitColor").isToggled()) {
                    if (CustomHitColor.rainbowHits.getBooleanValue()) {
                        int col = Chams.getRainbow(6500, -15);
                        float red2 = (col >> 16 & 255) / 255.0F;
                        float green2 = (col >> 8 & 255) / 255.0F;
                        float blue2 = (col & 255) / 255.0F;
                        this.brightnessBuffer.put(red2);
                        this.brightnessBuffer.put(blue2);
                        this.brightnessBuffer.put(green2);
                        this.brightnessBuffer.put(CustomHitColor.damageAlpha.getSliderValue().floatValue());
                    } else {


                        float sat = CustomHitColor.damageSaturation.getSliderValue().floatValue();
                        Color c = Color.getHSBColor(CustomHitColor.damageHue.getSliderValue().floatValue(), sat, 1.0F);
                        float red = c.getRed() / 255.0F;
                        float blue = c.getBlue() / 255.0F;
                        float green = c.getGreen() / 255.0F;
                ...
                if (Config.isShaders()) {
                    ...
                    float blue = c.getBlue() / 255.0F;
                    float green = c.getGreen() / 255.0F;
                    Shaders.setEntityColor(red, green, blue, 0.3F);
